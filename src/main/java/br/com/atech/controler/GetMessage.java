		...
		label197: try {
			...
			if (responseCode == 200) {
				...
				if (buffer.toString().trim().length() > 0) {
					msgList = getMessages(buffer.toString().trim());
					break label197;
				}
			} else {
...
//		}
	...
	public String getParametros(int posicao, String nome) {
		...
		return localidades + tipoMsg + dataInicial + dataFinal;
	}
	...
	public List<Aerodromo> montaLocalidades() {
		...
		try {
			...
			String sql = "select cd_localidade, latitude, longitude,  altitude " + 
					"     from TB_LOCALIDADE"
	...
	public List<Aerodromo> getAerodromos(){
		List<Aerodromo> listaAer = new ArrayList<>();
		Aerodromo aer1 = criarAerodromo(1, 1, "SBSP", 1);
		Aerodromo aer2 = criarAerodromo(1, 1, "SBAR", 1);
		listaAer.add(aer1);
		listaAer.add(aer2);
		return listaAer;
	}
